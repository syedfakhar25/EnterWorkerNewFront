...
@Component({
  selector: 'app-project-details-company-worker',
  templateUrl: './project-details-company-worker.component.html',
  styleUrls: ['./project-details-company-worker.component.css'],
  animations: [
    trigger("myTrigger", [
      state(
        "fadeInFlash",
...
export class ProjectDetailsCompanyWorkerComponent implements OnInit {
  ...
  constructor(public router: Router, public toast: ToasterService, private http: HttpClient,
    ...
    if (this.role == 'company worker' || this.role==undefined) {
      this.getProjectdetatails();
      this.getSteps()
      this.getProjectDrawing()
      this.getProjectOffer()
  ...
  ngOnInit() {
    this.getAllSuperAdminEmployees();
    this.getCTeam();
  ...
  getProjectAssignedComapnies() {
    ...
    this.http.get('https://cloneback.turnkey.no/api/manager/get-projects_company-workers/' + this.data, { headers: headers }).subscribe((data: any) => {
      console.log(data);
      this.allProjectComapnies = data[0];
      this.allProjectComapnies = this.allProjectComapnies.filter((data: { id: any; })=>data.id==this.shared?.customerData?.company)
    },
      (error: any) => {
        console.log('oops', error)
  ...
  getTeamMembers() {
    ...
    headers.append('Content-Type', 'multipart/form-data');
    headers.append('Accept', 'application/json');
    this.http.get('https://cloneback.turnkey.no/api/admin/team-for-company/'+this.data + `?by_company=${this.shared?.customerData?.company}`, { headers: headers }).subscribe((data: any) => {
      console.log(data);
      // let a: any[] = [];
  ...
  getAllSelectedCompanyEmployees() {
    ...
    this.http.get('https://cloneback.turnkey.no/api/admin/team-for-company/'+this.data + `?by_company=${this.shared?.customerData?.company}`, { headers: headers }).subscribe((data: any) => {
      this.selectedCompanyEmployees = data[0];
    },
      (error: any) => {
        console.log('oops', error)
